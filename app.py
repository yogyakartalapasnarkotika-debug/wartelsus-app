...
# 1. Konfigurasi Halaman & CSS Cetak
st.set_page_config(
...
with st.sidebar:
    ...
    if mode_aplikasi == "Formulir Input Laporan":
        ...
        st.markdown("**Total Pendapatan (Rupiah)**")
        total_pendapatan = st.number_input("Total Pendapatan (Rp)", min_value=0.0, value=59000000.0, step=1000000.0, format="%.2f", label_visibility="collapsed")
        st.caption(f"Formatted: **Rp {fmt(total_pendapatan)}**")

        st.markdown("---")
        st.subheader("📝 Daftar Biaya Operasional")

        # Dynamic List Entry
        biaya_temp = []
        for idx, item in enumerate(st.session_state.list_biaya):
            st.caption(f"Item #{idx+1}")
            c_nama, c_nom, c_del = st.columns([4.5, 4.5, 1])
            with c_nama:
                nama_val = st.text_input(f"Nama #{idx+1}", value=item["nama"], key=f"name_{idx}", label_visibility="collapsed", placeholder="Nama Biaya")
            with c_nom:
                nom_val = st.number_input(f"Nom #{idx+1}", value=float(item["nominal"]), key=f"nom_{idx}", label_visibility="collapsed", format="%.2f", step=50000.0)
            with c_del:
                if st.button("🗑️", key=f"del_{idx}"):
                    st.session_state.list_biaya.pop(idx)
                    st.rerun()
            biaya_temp.append({"nama": nama_val.upper(), "nominal": nom_val})

        st.session_state.list_biaya = biaya_temp

        # Tombol Tambah Baris
        if st.button("➕ Tambah Baris Biaya", use_container_width=True):
            st.session_state.list_biaya.append({"nama": "", "nominal": 0.0})
            st.rerun()
...
# 5. Penghitungan Laba Rugi & Distribusi[cite: 4, 5]
...
# Generasi HTML Hal 1 (Tanpa Indentasi awal baris untuk mencegah markdown code-block bug)[cite: 4]
...
# Generasi HTML Hal 2 (Tanpa Indentasi awal baris)[cite: 5]
